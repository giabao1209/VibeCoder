...
function clampChapterIndex(tab, index) {
  return Math.min(Math.max(0, Number(index) || 0), Math.max(0, tab.chapters.length - 1));
}

function scrollReaderTo(targetId = null, behavior = 'smooth') {
  const scroller = els.readerScroll;
  if (!scroller) return;

  const doScroll = () => {
    if (!targetId || targetId.startsWith('document-intro') || targetId === 'document-root') {
      scroller.scrollTo({ top: 0, behavior });
      return;
    }

    const target = els.document.querySelector(`#${cssEscape(targetId)}`);
    if (!target) {
      scroller.scrollTo({ top: 0, behavior });
      return;
    }
    ...
    const top = Math.max(0, scroller.scrollTop + targetRect.top - scrollerRect.top - 22);
...
function goToChapter(index, anchorId = null, behavior = 'smooth') {
  const tab = getActiveTab();
  if (!tab || !tab.chapters.length) return;

  tab.currentChapter = clampChapterIndex(tab, index);
  const chapter = tab.chapters[tab.currentChapter];
  saveSession();
  renderToc();
  renderReader();

  const targetId = anchorId || (state.mode === 'continuous' ? chapter.id : null);
  scrollReaderTo(targetId, behavior);
}

function moveChapter(delta) {
  const tab = getActiveTab();
  if (!tab || !tab.chapters.length) return;
  const next = clampChapterIndex(tab, tab.currentChapter + delta);
  if (next === tab.currentChapter) return;
  goToChapter(next);
}
  ...
  tab.chapters.forEach((chapter, index) => {
    ...
    if (index === tab.currentChapter && chapter.outline.length) {
      ...
      for (const item of chapter.outline) {
        ...
        child.dataset.chapterIndex = String(index);
        child.dataset.anchorId = item.id || '';
        child.textContent = item.title;
        child.addEventListener('click', (event) => {
          event.preventDefault();
          event.stopPropagation();
          goToChapter(index, item.id);
        });
...
els.modeSelect.addEventListener('change', () => {
  ...
  const tab = getActiveTab();
  if (tab) {
    const targetId = state.mode === 'continuous' ? tab.chapters[tab.currentChapter]?.id : null;
    scrollReaderTo(targetId, 'auto');
  }
});
